...
window.onload=function(){
    ...
    if (snapshot.val()==0)     $('#bomba').bootstrapToggle('off');
    else if (snapshot.val()==1) $('#bomba').bootstrapToggle('on');
       variablesdecontrol.Bomba=snapshot.val();
    });

    firebase.database().ref('Control/AireC').once('value',function (snapshot) {
        if (snapshot.val()==0)     $('#acalient').bootstrapToggle('off');
        else if (snapshot.val()==1) $('#acalient').bootstrapToggle('on');
        variablesdecontrol.AireC=snapshot.val();

    });
    firebase.database().ref('Control/AireF').once('value',function (snapshot) {
        if (snapshot.val()==0)     $('#aFrio').bootstrapToggle('off');
        else if (snapshot.val()==1) $('#aFrio').bootstrapToggle('on');
